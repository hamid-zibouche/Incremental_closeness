...
from generator import generate_barabasi_albert_actions
...
def run_classical_benchmark(actions):
    ...
    Returns:
        tuple: (temps_execution, closeness_finale)
    """
    G = DynamicGraph()
    ...
    for action in actions:
        ...
        # Calculer closeness APRÈS CHAQUE ACTION (comme l'incrémental)
...
def verify_correctness(clos_class, clos_incr, tolerance=1e-5):
    ...
    Args:
        clos_class: Closeness de la méthode classique
        clos_incr: Closeness de la méthode incrémentale
        tolerance: Tolérance pour la comparaison
        
    Returns:
        tuple: (correct, max_difference, num_differences)
    """
    differences = []
...
def run_benchmark(graph_sizes, num_runs=3):
    ...
    Args:
        graph_sizes: Liste de tuples (n_nodes, m_edges_per_node)
        num_runs: Nombre de runs par configuration pour moyenner
...
def save_results(results, output_file):
    ...
    Args:
        results: Liste des résultats du benchmark
